...
Mục đích màn hình: Là điểm chạm đầu tiên của khách hàng, giúp giới thiệu các bộ phim đang chiếu, sắp chiếu, các chương trình khuyến mãi và điều hướng người dùng bắt đầu luồng đặt vé nhanh chóng.
...
Header: Logo rạp phim, thanh tìm kiếm thông minh, bộ lọc vị trí (Tỉnh/Thành phố), nút Đăng nhập/Đăng ký.
...
Mục đích màn hình: Cung cấp thông tin đầy đủ, trực quan về một bộ phim cụ thể (nội dung, trailer, đánh giá) và là nơi để khách hàng lựa chọn ngày xem, rạp xem và suất chiếu phù hợp với lịch trình cá nhân.
...
Movie Info Section: Poster lớn, tên phim (Tiếng Việt & Tiếng Anh), thời lượng (phút), ngày khởi chiếu, quốc gia, điểm số đánh giá từ người dùng.

Synopsis Section: Đoạn văn ngắn tóm tắt nội dung cốt truyện và danh sách hình ảnh/tên các diễn viên chính, đạo diễn.

Booking Filter Bar: Thanh chọn ngày (Thứ, Ngày/Tháng dạng trượt ngang) và Dropdown chọn khu vực/Rạp.

Showtime Section: Danh sách các rạp còn suất chiếu. Dưới mỗi tên rạp chia theo loại phòng/định dạng phim và danh sách các block thời gian suất chiếu cụ thể (Ví dụ: 09:30, 14:15, 20:45).

Acceptance Criteria (Tiêu chí nghiệm thu):

Các suất chiếu của quá khứ (so với giờ hiện tại của hệ thống) bắt buộc phải bị ẩn hoặc làm mờ (disabled), không cho phép người dùng tương tác ấn vào.
...
Mục đích màn hình: Cho phép khách hàng quan sát sơ đồ phòng chiếu theo thời gian thực (Real-time) để lựa chọn vị trí ngồi mong muốn và chọn mua thêm các combo bắp nước đi kèm trước khi thanh toán.
...
Seat Map Grid: Ma trận các ô ghế được ký hiệu theo hàng chữ (A, B, C,...) và cột số (1, 2, 3,...). Màu sắc phân biệt rõ ràng: Ghế thường (Xám), Ghế VIP (Vàng/Đỏ), Ghế đôi (Hồng), Ghế đã có người đặt (Có dấu X hoặc màu xám đậm), Ghế đang chọn (Xanh lá).

Seat Legend: Chú thích chi tiết ý nghĩa của các loại màu sắc ghế ở phía dưới sơ đồ.
...
Mục đích màn hình: Cung cấp hóa đơn chi tiết (Invoice) tổng kết toàn bộ đơn hàng và cung cấp các phương thức thanh toán an toàn để khách hàng tiến hành trả tiền vé.
...
Payment Methods List: Danh sách các Radio Button chọn phương thức thanh toán phổ biến: Ví điện tử (Momo, ZaloPay, ShopeePay), Thẻ nội địa / Cổng VNPAY (Quét mã QR Code ngân hàng), Thẻ quốc tế (Visa/Mastercard/JCB).
...
Mục đích màn hình: Xác nhận giao dịch thành công, cung cấp thông tin vé điện tử cùng mã QR/Mã vạch để khách hàng sử dụng trực tiếp khi đến rạp nhận vé cứng hoặc vào phòng chiếu.
...
Digital Ticket Card: Thiết kế giả lập như một chiếc vé giấy xinh xắn bao gồm: Mã QR Code / Mã vạch (Barcode) kích thước lớn nằm ở vị trí trung tâm, Mã đặt vé (Booking ID), Thông tin chi tiết (Tên phim, Thời gian, Rạp, Phòng chiếu, Số ghế, Combo bắp nước đi kèm), Tổng tiền đã thanh toán (hiển thị trạng thái màu xanh "Đã thanh toán").
