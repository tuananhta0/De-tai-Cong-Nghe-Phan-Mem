/* ====================================================================
   X CINEMA - SCRIPT NẠP DỮ LIỆU MẪU (INSERT) - BẢN GỘP HOÀN CHỈNH
   ====================================================================
   Chạy SAU khi đã chạy xong 01_create_database.sql.

   File này gộp và viết lại từ đầu dữ liệu mẫu của:
     - SQLQuery1.sql               (30 phim, 5 rạp, 10 phòng, 960 ghế,
                                     52 suất chiếu, 10 KM, 10 tin, 3 combo)
     - tao5taikhoanadmin-cumrap.sql (5 tài khoản admin cụm rạp, đã gộp
                                     thẳng vào bước tạo Users bên dưới,
                                     không cần chạy script rời nữa)

   Thứ tự nạp tuân theo đúng phụ thuộc khóa ngoại:
   Roles -> Cinemas -> Users -> Movies -> Promotions -> News ->
   CinemaHalls -> Seats -> Showtimes -> ShowtimeSeats -> Combos
   -> (cuối cùng) tạo trigger chống trùng giờ chiếu.
   ==================================================================== */


-- ============================================================
--  BƯỚC 1: VAI TRÒ (ROLES) - đã gộp sẵn 'cinema_admin'
-- ============================================================
INSERT INTO dbo.Roles (RoleCode, RoleName) VALUES
('admin',        N'Quản Lý'),
('cinema_admin',  N'Quản lý cụm rạp'),
...
-- ============================================================
--  BƯỚC 3: TÀI KHOẢN
--  3.1: Admin tổng / Nhân viên / Khách hàng mẫu
-- ============================================================
-- 10.2: Tài khoản mẫu
...
-- 3.2: 5 tài khoản admin cụm rạp - mỗi tài khoản gắn đúng 1 trong 5 cụm rạp
--      vừa tạo ở BƯỚC 2. Mật khẩu mặc định: "admin" (đổi lại sau khi đăng
--      nhập lần đầu). Trước đây đây là script vá tao5taikhoanadmin-cumrap.sql,
--      nay gộp thẳng vào bước seed Users.
INSERT INTO dbo.Users
    (Username, PasswordHash, FullName, Email, Phone, RoleID, RoleCode, Avatar, MembershipId, Points, FavoriteMoviesJson, CinemaID)
SELECT
    na.Email,
    HASHBYTES('SHA2_256', CAST('admin' AS VARCHAR(255))),
    na.FullName,
    na.Email,
    na.Phone,
    (SELECT RoleID FROM dbo.Roles WHERE RoleCode = 'cinema_admin'),
    'cinema_admin',
    'https://api.dicebear.com/7.x/bottts/svg?seed=' + na.Email,
    'X-CADMIN-' + CAST(c.CinemaID AS VARCHAR(10)),
    0,
    '[]',
    c.CinemaID
FROM (VALUES
    (N'Quản lý Cụm rạp Hoàn Kiếm',  'cumrap.hoankiem@xcinema.vn',   '0900000001', N'X Cinema Hoàn Kiếm'),
    (N'Quản lý Cụm rạp Cầu Giấy',   'cumrap.caugiay@xcinema.vn',    '0900000002', N'X Cinema Cầu Giấy'),
    (N'Quản lý Cụm rạp Thanh Xuân', N'cumrap.thanhxuan@xcinema.vn', '0900000003', N'X Cinema Thanh Xuân'),
    (N'Quản lý Cụm rạp Hà Đông',    'cumrap.hadong@xcinema.vn',     '0900000004', N'X Cinema Hà Đông'),
    (N'Quản lý Cụm rạp Long Biên',  'cumrap.longbien@xcinema.vn',   '0900000005', N'X Cinema Long Biên')
) AS na(FullName, Email, Phone, CinemaName)
JOIN dbo.Cinemas c ON c.CinemaName = na.CinemaName;
GO
...
-- ============================================================
--  BƯỚC 5: KHUYẾN MÃI (10 khuyến mãi)
-- ============================================================
...
-- ============================================================
--  BƯỚC 7: PHÒNG CHIẾU (mỗi rạp 2 phòng)
-- ============================================================
...
-- ============================================================
--  BƯỚC 8: GHẾ - 8 hàng (A-H) x 12 ghế/hàng, khớp SeatMap.tsx:
--    Hàng A,B,C => 'Thường'
--    Hàng D,E,F,G => 'VIP'
...
-- ==================================================================
--  BƯỚC 9: SUẤT CHIẾU - tái tạo đúng thuật toán generateShowtimes()
--  gốc trong seedData.ts (vòng lặp rạp -> phim -> ngày, công thức
--  modulo y hệt), đảm bảo MỌI phim đang chiếu đều có suất ở MỌI rạp.
--  Ngày dùng GETDATE() (hôm nay) và GETDATE()+1 (ngày mai) - luôn
--  đúng thời điểm chạy script.
-- ==================================================================
...
-- ==================================================================
--  BƯỚC 10: COMBO BẮP NƯỚC (3 combo)
-- ==================================================================
...
-- ============================================================
--  BƯỚC 11: TRIGGER CHỐNG TRÙNG GIỜ CHIẾU TRONG CÙNG 1 PHÒNG
...
--
--  LƯU Ý: Trigger được tạo SAU khi đã seed xong dữ liệu mẫu ở trên, vì
--  dữ liệu demo (sinh theo đúng thuật toán gốc của frontend) không tính
--  đến thời lượng phim nên một vài suất có thể "đè giờ" nhau về mặt kỹ
--  thuật - đó là điều chấp nhận được cho dữ liệu demo. Từ đây về sau,
--  mọi suất chiếu được tạo/sửa qua ứng dụng (trang Admin) sẽ bị trigger
--  này chặn nếu trùng giờ trong cùng phòng.
-- ============================================================
...
SELECT SeatType, COUNT(*) AS SoLuong FROM dbo.Seats GROUP BY SeatType;    -- kỳ vọng: Thường=288, VIP=384, Sweetbox=120 (tổng 10 phòng)
SELECT RoleCode, COUNT(*) AS SoTaiKhoan FROM dbo.Users GROUP BY RoleCode; -- kỳ vọng: admin=1, cinema_admin=5, employee=1, customer=1
GO

PRINT N'Hoàn tất nạp dữ liệu X Cinema: 30 phim, 5 rạp, 10 phòng, 960 ghế (12/hàng, đủ 3 loại ghế), suất chiếu phủ mọi phim/mọi rạp, 10 KM, 10 tin, 3 combo, 1 admin tổng + 5 admin cụm rạp + 1 nhân viên + 1 khách hàng mẫu, kèm trigger chống trùng giờ.';
GO
